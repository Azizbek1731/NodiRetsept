...
module.exports = function authRoutes(loginLimiter) {
  ...
  router.post('/login', loginLimiter, (req, res) => {
    ...
    const fail = (msg) => res.status(401).render('login', {
      title: 'Tizimga kirish', bodyClass: 'auth-page',
      next: nextUrl, error: msg, username,
    });

    const user = auth.findUserByUsername(username);
    if (!user || !auth.verifyPassword(password, user.password_hash)) {
      return fail('Login yoki parol noto\'g\'ri.');
    }
    if (!user.is_active) return fail('Ushbu hisob faolsizlantirilgan. Administratorga murojaat qiling.');

    req.session.regenerate((err) => {
      if (err) return fail('Sessiyani boshlashda xatolik.');
